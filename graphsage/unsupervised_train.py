...
def train(train_data, test_data=None):
    ...
    with tf.name_scope("train"):
        ...
        summary_train = tf.summary.merge([summary_train_loss, summary_train_mrr])
    with tf.name_scope("val"):
        summary_val_loss = tf.summary.scalar('loss', model.loss)
        summary_val_mrr = tf.summary.scalar('mrr', model.mrr)
        summary_val = tf.summary.merge([summary_val_loss, summary_val_mrr])
    ...
    for epoch in range(FLAGS.epochs):
        ...
        while not minibatch.end():
            ...
            t = time.time()
            # Training step
            outs = sess.run([summary_train, model.opt_op, model.loss, model.ranks, model.aff_all,
